    ...
    <nav class="max-w-7xl px-5 md:fixed top-0 z-[98] w-screen backdrop-blur-md bg-[#121212] bg-opacity-80">
      <div class="container mx-auto flex flex-wrap items-center justify-between">
        <button @click="redirectToHome" class="flex">
          <span class="self-center text-lg text-[#ffdb70] font-semibold whitespace-nowrap fadein-bot hover:text-amber-100">console.log("Arul");</span>
        </button>
        <div class="flex items-center space-x-4 md:order-2 fadein-bot">
          <a href="https://github.com/AhmadMnrl"><img class="w-9 rounded-full" src="https://seeklogo.com/images/G/github-logo-2E3852456C-seeklogo.com.png" alt="github"></a>
          <button @click="toggleTheme" class="focus:outline-none">
            <span v-if="isDarkMode" class="text-gray-300 hover:text-white">🌞</span>
            <span v-else class="text-gray-300 hover:text-white">🌜</span>
          </button>
        </div>
...
nav.dark a.router-link-exact-active {
  display: inline-flex;
  flex-direction: column;
  color: white;
  transition: color 0.3s;
...
nav.dark a.router-link-exact-active::after {
  display: inline-block;
  content: "";
  margin-top: 0.08em;
  width: 100%;
...
nav.dark a.router-link-exact-active:hover {
  color: white;
}
